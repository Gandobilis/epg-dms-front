...
const {email, password, remember, error, toggleRemember, login} = useLogin();
</script>
      ...
      <form @submit.prevent="login">
        ...
        <div class="flex justify-between items-center mt-4">
          <label class="flex items-center">
            <input type="checkbox" @change="toggleRemember" :checked="remember" class="checkbox checkbox-sm"/>
            <span class="ml-2 text-sm text-gray-700">დამახსოვრება</span>
          </label>
        </div>
        <button type="submit"
                class="w-full mt-6 py-2 px-4 text-white font-semibold rounded-md hover:bg-success glass bg-neutral">
          შესვლა
        </button>
      </form>
    </div>
